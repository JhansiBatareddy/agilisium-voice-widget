(function () {
  ...
  var WELCOME_MESSAGE = "Hi there! I'm your Agilisium AI assistant.";
  var OPENING_Q = "What brings you to Agilisium today?";
  var CONTACT_URL = "https://agilisium.com/contact-us";
  ...
  var ICON = {
    ...
    expand:   '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M15 3h6v6"/><path d="M9 21H3v-6"/><path d="M21 3l-7 7"/><path d="M3 21l7-7"/></svg>',
    collapse: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 14h6v6"/><path d="M20 10h-6V4"/><path d="M14 10l7-7"/><path d="M3 21l7-7"/></svg>',
    phoneOff: '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M10.68 13.31a16 16 0 0 0 3.41 2.6l1.27-1.27a2 2 0 0 1 2.11-.45 12.84 12.84 0 0 0 2.81.7 2 2 0 0 1 1.72 2v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07 19.5 19.5 0 0 1-3.33-2.67"/><path d="M5 5a2 2 0 0 1 2-2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L11 10.9"/><line x1="22" y1="2" x2="2" y2="22"/></svg>',
    volume:   '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><path d="M15.54 8.46a5 5 0 0 1 0 7.07"/><path d="M19.07 4.93a10 10 0 0 1 0 14.14"/></svg>',
    volumeOff:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polygon points="11 5 6 9 2 9 2 15 6 15 11 19 11 5"/><line x1="23" y1="9" x2="17" y2="15"/><line x1="17" y1="9" x2="23" y2="15"/></svg>'
  };
  ...
  function loadViewMode(){
    try { var v = sessionStorage.getItem("agx_view"); return v === "side" ? "side" : "centered"; } catch(_){ return "centered"; }
  }
  function saveViewMode(v){ try { sessionStorage.setItem("agx_view", v); } catch(_){} }

  var state = {
    open:false, muted:false, micMuted:false,
  ...
  /* ============================================================
     ...
     ============================================================ */
  /* --- Shadow DOM isolation (prevents host CSS from bleeding in) --- */
  // Derive css URL from the current <script src>
  var scriptEl = document.currentScript || (function(){
    var s = document.getElementsByTagName("script"); return s[s.length-1];
  })();
  var scriptSrc = (scriptEl && scriptEl.src) || "";
  var cssHref = scriptSrc.replace(/[^/]*(?:\?.*)?$/, "") + "chatbot.css";
  // Allow override: <script src="chatbot.js" data-css="https://.../chatbot.css">
  if (scriptEl && scriptEl.getAttribute("data-css")) cssHref = scriptEl.getAttribute("data-css");

  var host = document.createElement("div");
  host.id = "agilisium-chatbot-host";
  ...
  // Fallback if browser has no Shadow DOM: mount into host directly
  var mountTarget = shadow || host;
  ...
  root.className = "agx";
  root.style.cssText = "position:fixed!important;inset:0!important;width:100vw!important;height:100dvh!important;pointer-events:none!important;z-index:2147483000!important;isolation:isolate!important;";
  root.innerHTML = ''
    ...
    +   '<div class="agx-shell">'
    +     '<div class="agx-powered">'
    +       '<span class="agx-powered-orb">' + ICON.sparkles + '</span>'
    +       '<span class="agx-powered-text">Powered by <b>Agilisium</b></span>'
    +     '</div>'
    +     '<div class="agx-stage">'
    ...
    +         '<div class="agx-status-sub">Tap the mic to begin</div>'
    +       '</div>'
    +       '<div class="agx-controls">'
    +         '<a class="agx-contact" href="' + CONTACT_URL + '" target="_blank" rel="noopener">Contact us</a>'
    +         '<button class="agx-ctrl agx-mic" type="button" aria-label="Mute microphone">' + ICON.mic + '</button>'
    +         '<button class="agx-ctrl agx-speaker" type="button" aria-label="Mute AI voice">' + ICON.volume + '</button>'
    +         '<button class="agx-ctrl agx-endcall" type="button" aria-label="End call">' + ICON.phoneOff + '</button>'
    +       '</div>'
    +     '</div>'
    +     '<div class="agx-conv">'
    ...
    +           '</span>'
    +           '<button class="agx-head-btn agx-toggle" type="button" aria-label="Toggle view">' + ICON.expand + '</button>'
    ...
    +       '</form>'
    +       '<div class="agx-disclaimer">Responses may be inaccurate. For anything important, please <a href="' + CONTACT_URL + '" target="_blank" rel="noopener">contact us</a>.</div>'
    +     '</div>'
    +   '</div>'
    + '</div>';
  ...
  var speakerBtn  = root.querySelector(".agx-speaker");
  var endBtn      = root.querySelector(".agx-endcall");

  speakerBtn.addEventListener("click", function(){
    state.muted = !state.muted;
    speakerBtn.classList.toggle("agx-off", state.muted);
    speakerBtn.innerHTML = state.muted ? ICON.volumeOff : ICON.volume;
    speakerBtn.setAttribute("aria-label", state.muted ? "Unmute AI voice" : "Mute AI voice");
    if (state.muted) interruptAudio();
  });
  endBtn.addEventListener("click", function(){ handleClose(); });
  ...
  function updateAvatarState(){
    ...
    statusSub.textContent = state.listening
      ...
      : state.micMuted ? "Tap the mic to begin" : "";
  ...
  function applyViewMode(){
    if (!shellEl) return;
    shellEl.classList.toggle("agx-docked", state.viewMode === "side");
    if (toggleBtn) {
      toggleBtn.innerHTML = state.viewMode === "side" ? ICON.expand : ICON.collapse;
      toggleBtn.setAttribute("aria-label", state.viewMode === "side" ? "Expand to center" : "Dock to side");
    }
  }
  function toggleView(){
    state.viewMode = state.viewMode === "side" ? "centered" : "side";
  ...
  function openModal(){
    ...
    checkConn();
    // spoken welcome on every open — seed BOTH bubbles, then speak sequentially
    if (!state.welcomed) {
      state.welcomed = true;
      state.messages.push({ role:"assistant", content:WELCOME_MESSAGE, ts:Date.now(), voice:true });
      state.messages.push({ role:"assistant", content:OPENING_Q,       ts:Date.now()+1, voice:true });
      renderMessages();
      speakCanned(WELCOME_MESSAGE + " " + OPENING_Q);
    }
    startAmbientVAD();
  }
  function handleClose(){
    state.open = false; state.welcomed = false;
  ...
  /* Keep every wheel/touch gesture over the fixed panel inside its message scroller. */
  shellEl.addEventListener("wheel", function(e){
    if (!state.open) return;
    if (scrollEl.scrollHeight > scrollEl.clientHeight) scrollEl.scrollTop += e.deltaY;
    e.preventDefault();
    e.stopPropagation();
  }, { passive:false });
  ...
  shellEl.addEventListener("touchstart", function(e){
    panelTouchY = e.touches && e.touches.length ? e.touches[0].clientY : null;
  }, { passive:true });
  shellEl.addEventListener("touchmove", function(e){
    if (!state.open || panelTouchY === null || !e.touches || !e.touches.length) return;
    var nextY = e.touches[0].clientY;
    if (scrollEl.scrollHeight > scrollEl.clientHeight) scrollEl.scrollTop += panelTouchY - nextY;
    panelTouchY = nextY;
    e.preventDefault();
    e.stopPropagation();
  }, { passive:false });
  shellEl.addEventListener("touchend", function(){ panelTouchY = null; }, { passive:true });
  shellEl.addEventListener("touchcancel", function(){ panelTouchY = null; }, { passive:true });
